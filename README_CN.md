...
Mano-P 基于 Mano 项目的完整技术框架（参见 Mano 技术报告），采用 Mano-Action 双向自强化学习方法、三阶段渐进式训练（SFT → 离线强化学习 → 在线强化学习）、“思考-行动-验证”循环推理机制以及闭环数据循环系统，从而实现高精度的 GUI 理解和操作能力。边缘版本通过混合精度量化、视觉标记剪枝和边缘推理自适应进行优化，使大规模参数模型能够在 Mac mini/Macbook/算力棒 等边缘设备上高效运行。
...
Mano-P 基于完整的 Mano 项目技术体系（详见 [Mano Technical Report](https://arxiv.org/abs/2509.17336)），采用 Mano-Action 双向自增强学习方法，通过三阶段渐进式训练（SFT → 离线RL → 在线RL）和"思考-行动-验证"循环推理机制，配合闭环数据循环系统，实现了高精度的 GUI 理解和操作能力。端侧版本通过混合精度量化、视觉 Token 剪枝等优化，使大参数量模型能够在算力棒等端侧设备上高效运行。
...
- **灵活的推理模式**：
  d额 - **本地模式**：模型在本地运行，数据不出设备，快速响应 - 直接在 Mac mini/MacBook（M4 芯片及以上，32GB+ 内存）上运行 - 或使用 Mano-P 算力棒（通过 USB 4.0 连接）
